...
class TestMovingPandasAnalyzer(TestCase):
    ...
    def test_it_should_analyze_input4_LatLon(self):
        ...
        # verify
        self.assertEqual(actual['positions_total_number'], 7770)
        self.assertEqual(actual['timestamps_range'], ['2013-08-07 18:27:02', '2015-02-19 13:42:27'])
        self.assertEqual(actual['tracks_total_number'], 3)
        self.assertEqual(actual['track_names'], ['X742..deploy_id.56853924.', 'X746..deploy_id.20813885.', 'X749..deploy_id.20813892.'])
        self.assertEqual(actual['number_positions_by_track'], [['X742..deploy_id.56853924.', 3265], ['X746..deploy_id.20813885.', 3010], ['X749..deploy_id.20813892.', 1495]])
        self.assertEqual(actual['projection'], 'EPSG:4326')
        self.assertEqual(actual['positions_bounding_box'], [3.7317, 49.417633, 53.619633, 69.156667])
        self.assertEqual(actual['track_attributes'], ['sensor_type_id', 'barometric_pressure', 'data_decoding_software', 'eobs_activity', 'eobs_activity_samples', 'eobs_battery_voltage', 'eobs_fix_battery_voltage', 'eobs_horizontal_accuracy_estimate', 'eobs_key_bin_checksum', 'eobs_speed_accuracy_estimate', 'eobs_start_timestamp', 'eobs_status', 'eobs_temperature', 'eobs_type_of_fix', 'eobs_used_time_to_get_fix', 'gps_dop', 'gps_satellite_count', 'ground_speed', 'gt_tx_count', 'heading', 'height_above_ellipsoid', 'height_raw', 'import_marked_outlier', 'manually_marked_outlier', 'tag_voltage', 'timestamp', 'event_id', 'visible', 'individual_name_deployment_id', 'timestamp_tz', 'geometry'])
        self.assertEqual(actual['animals_total_number'], 'total number of animals is not available')
        self.assertEqual(actual['animal_names'], 'no appropriate animal names available')
        self.assertEqual(actual['sensor_types'], ['GPS'])
        self.assertEqual(actual['taxa'], 'no appropriate taxa names available')
